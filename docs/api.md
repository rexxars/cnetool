...
File formats and the scripting system referenced throughout are documented separately.
...
- [Menu profile (`menuinfo.dat`)](#menu-profile-menuinfodat)
...
Most of the game's assets live in `.dat` archives: a `uint32` entry count, a table of contents of fixed 17-byte records (13-byte NUL-terminated name + `uint32` absolute blob offset), then the data blobs. Two container layouts exist:
...
Rebuilds a standalone TGA file for a texture entry by prepending the constant 8 header bytes the archive strips. Throws if the entry is not a supported texture; check with `getTextureInfo` first.
...
CE textures are uncompressed true-color TGAs (24-bit opaque or 32-bit with alpha). These helpers convert to and from PNG for editing, and encode/decode the raw pixel form (`RawImage`: top-down rows, 3 or 4 channels). Transparency rules and texture size limits are covered in the formats reference.
...
`objects.dat` entries ("projects") are 3D models: a shared vertex array plus up to three render LOD layers and an optional collision hull. These functions parse projects into a plain `Mesh` (`{vertices, faces}`), export to OBJ/MTL or glTF/GLB, and import edited OBJ back into the game's format.
...
Parses a project's detect/collision mesh: the low-poly hull appended after the render LODs, with its own vertex array. Returns `null` when the project has none (simple/static entries).
...
Parses the texture-name table from `objects.dat` (a `uint32` count + 13-byte name records after the project TOC). A mesh face's `texId` indexes this table, giving the texture's source filename to resolve against a texture archive.
...
A level's placed objects live in its binary `data1.bin` or the text `World.dat`; the terrain is a regular `objects.dat` project named by the level's `MAINSCR.SCR`. `assembleLevel` combines all of it into world-space meshes ready for `meshesToObj` or `meshesToGlb`.
...
- `controllable`: vehicles and turrets are placed under logical names whose own project is an empty stub (the engine attaches the body at runtime). Pass `true` to substitute the built-in `controllableGeometry` part map so they render, or pass your own `ControllableGeometryMap` to override it. Default `false` leaves them as stubs.
...
Parses a level's text `World.dat` (the human-readable twin of `data1.bin`) into placements. Each block's `Dof`/`Up`/`Right` basis vectors map onto the same 9-value rotation matrix as a `Placement` (row order `[Dof, Up, Right]`, verified byte-identical to `data1.bin`), so the result drops straight into `transformMesh`/`assembleLevel`. Each entry also carries its keyword: `Name` places an object, `Dele` is a removal directive seen in patch files.
...
Vehicles and turrets ("controllables") are placed under logical project names whose `objects.dat` entry is an empty stub; the engine assembles the real multi-part body at runtime. These exports encode the reverse-engineered part layouts so static exports can render them, plus the `.anm` vertex-animation format some parts use.
...
Several game files are `Key:Value` text (`MOBJS.DAT`, `KEYCONF.DAT`, ...) and two, `data3.bin`/`data4.bin`, are byte-obfuscated stat tables the engine reads as text in fixed-size chunks.
...
### `packStatSlot(text: string): Uint8Array`

Packs one `key:value` record (no trailing newline - this appends it) into a single obfuscated `STAT_CHUNK_SIZE`-byte slot, laid out exactly as the engine's stock tables store it: the obfuscated line followed by a literal `0x00` terminator, then zero filler. The primitive `formatStatTable` and the typed table serializers below build on this. Throws `RangeError` if the line does not fit alongside the terminator, or if a byte would obfuscate to `0x00` and prematurely end the slot.

### `parseUnitTable(data: Uint8Array): Unit[]` / `serializeUnitTable(units: Unit[]): Uint8Array`

Typed read/write for the obfuscated **unit** stat table (`data3.bin` / `mdata3.bin`). Each unit is a fixed field sequence: `Name`, an optional `Armor` (a `UnitArmor`; the engine defaults units that omit it to `none`), `Health` (`%d`), and `Firedelay` (`%f` - note the lowercase `d`). `parseUnitTable` throws on a missing or out-of-order field or an unrecognized `Armor` value; `serializeUnitTable` is the inverse, always a multiple of `STAT_CHUNK_SIZE` bytes.

### `parseWeaponTable(data: Uint8Array): WeaponTable` / `serializeWeaponTable(table: WeaponTable): Uint8Array`

Typed read/write for the obfuscated **weapon** table (`data4.bin` / `mdata4.bin`): a 4-slot header (an ignored `Armor` comment, then the `gas`/`bullet`/`shell` damage-vs-armor rows as `ArmorDamage` percentages) followed by 7-slot weapon records (`Name`, `AmmoSpeed`, `FireDelay` - capital `D` -, `Damage`, `AmmoType`, `WeaponLength`, `Sound`). The engine selects a weapon by its **position** in the file, exposed as `Weapon.index`; `serializeWeaponTable` writes weapons in array order regardless of `index`.

```ts
import {readFile, writeFile} from 'node:fs/promises'
import {parseWeaponTable, serializeWeaponTable} from 'cnetool'

const table = parseWeaponTable(await readFile('data4.bin'))
table.weapons[0]!.damage = 50
await writeFile('data4.bin', serializeWeaponTable(table))
```

### `AMMO_TYPES`, `UNIT_ARMORS`

The canonical `AmmoType` (`'bullet' | 'gas' | 'shell'`) and `UnitArmor` (`'heavy' | 'light' | 'none'`) value sets, for narrowing/validating parsed JSON against.

## Menu profile (`menuinfo.dat`)

The persisted menu profile/options file: an 8-byte header (uncompressed/compressed size) wraps a two-layer cipher (add/subtract a cyclic key) around a zlib-deflated payload of fixed 272-byte blocks (`PlayInfo`, `OptionsMenu`, ...) found by tag. `parseMenuInfo`/`formatMenuInfo` give a typed read/patch API without needing the layout; `decodeMenuInfo`/`encodeMenuInfo` expose the raw plaintext payload for lower-level access.

### `parseMenuInfo(data: Uint8Array): MenuInfo`

Decodes and parses `menuinfo.dat` into a typed `MenuInfo`: player/host name, team, game mode, network protocol, last-connected server, save slot, renderer, resolution, detail/graphics levels, audio, language, and subtitles. Numeric enum fields keep their raw byte values (the meanings are documented on `MenuInfo`).

### `formatMenuInfo(original: Uint8Array, patch: MenuInfoPatch): Uint8Array`

Rewrites `menuinfo.dat` with a set of field overrides, the inverse of `parseMenuInfo`: only the fields present in `patch` are overwritten (string fields NUL-terminated and zero-filled); every other block and any unmapped bytes round-trip untouched.

```ts
import {readFile, writeFile} from 'node:fs/promises'
import {formatMenuInfo, parseMenuInfo} from 'cnetool'

const data = await readFile('menuinfo.dat')
console.log(parseMenuInfo(data).playerName)
await writeFile('menuinfo.dat', formatMenuInfo(data, {playerName: 'Ace'}))
```

### `decodeMenuInfo(data: Uint8Array): Uint8Array` / `encodeMenuInfo(plain: Uint8Array): Uint8Array`

Lower-level codec beneath `parseMenuInfo`/`formatMenuInfo`: decodes `menuinfo.dat` to its raw 816-byte plaintext payload, or re-encodes it back. `encodeMenuInfo`'s output round-trips through `decodeMenuInfo` but is not byte-identical to a game-written file (the deflate stream differs).

## Localization

The dialogue and briefing files hold the game's localized text.
...
Small per-level binary files, and the level's ambient configuration recovered from its main script.
...
Read the community master list, discover LAN hosts, and query a server's live status over the GameSpy protocol. Unlike the rest of the API these do network I/O and are **Node-only** (UDP via `node:dgram`, HTTP via `fetch`). The corresponding CLI is `cnetool server`.
...
Parses one 24-byte `'D'` beacon into `{name, numPlayers, maxPlayers}`, or `null` if it is not a beacon.
...
`queryServer` is composed from four lower-level building blocks, each useful on its own if you want to talk to the GameSpy query protocol directly:

### `parseQueryPacket(datagram: Uint8Array): Record<string, string>`

Parses one GameSpy-1 reply datagram (`\key\value\key\value\...`) into a flat key→value map.

### `createReassembler(expectedQueries: number): {push: (datagram: Uint8Array) => boolean, fields: () => Record<string, string>}`

Reassembles a multi-packet GameSpy reply for one or more concurrent queries. CE tags every reply packet with `\queryid\<id>.<segment>`; the packet carrying `\final\` gives the total segment count for that id (packets can arrive out of order over UDP, so completion is counted, not sequenced). `push(datagram)` feeds one packet and returns `true` once `expectedQueries` ids have completed; `fields()` returns the fields merged so far, across every id.

### `parsePlayers(fields: Record<string, string>): GamePlayer[]`

Maps merged GameSpy reply fields (the `player_<n>`/`frags_<n>`/... rows, from either `\players\` or a bundled `\status\` reply) to the connected-player roster, sorted by index.

### `parseServerStatus(fields: Record<string, string>, ip: string, queryPort: number, ping?: number): GameServerStatus`

Assembles a `GameServerStatus` from merged `\status\` (+ optional `\players\`) fields. Throws if the reply lacks a `hostname` field (i.e. it wasn't a real status reply).
...
- `Unit` / `UnitArmor`: a typed `data3.bin` unit record (`name`, optional `armor`, `health`, `fireDelay`) and its armor class (`'heavy' | 'light' | 'none'`).
- `Weapon` / `WeaponTable` / `ArmorDamage` / `AmmoType`: a typed `data4.bin` weapon record (`index`, `name`, `damage`, `ammoType`, `ammoSpeed`, `fireDelay`, `weaponLength`, `sound`), the parsed table (`ammoDamage` + `weapons`), one damage-vs-armor row (`heavy`/`light`/`none` percentages), and the ammo category union (`'bullet' | 'gas' | 'shell'`).
...
- `MenuInfo`: a parsed `menuinfo.dat`: player/host name, team, game mode, network protocol, last-connected server, save slot, renderer, `resolution` (`width`/`height`/`depth`), detail/graphics levels, audio volumes/channels, language, and subtitles.
- `MenuInfoPatch`: the partial-`MenuInfo` shape `formatMenuInfo` accepts to overwrite specific fields.
...
- `QueryServerOptions` / `FetchIpListOptions` / `DiscoverLanOptions` / `FindServersOptions`: options for `queryServer`, `fetchIpList`, `discoverLanServers`, and `findServers` respectively (timeouts, an `AbortSignal`, and command-specific flags like `includePlayers` or `lan`).
