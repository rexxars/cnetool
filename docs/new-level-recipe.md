...
- **Placing:** `F12` console → `place <object>` (eg `place house`, `place flagbase`); numpad 8/2 = ±Z, 4/6 = ∓X, 9/3 = ∓Y, `0` reset, `5` **drop** (stamps a real copy and appends it to `World.dat` on the spot - not just on exit), hold Left-Shift for a coarser step; **shoot a placed object to delete it** (the engine rewrites that record's tag to `Dele:` in place and drops `faccache.bin`/`data1.bin`). Full scancode map and the RE confirmation are in [game-flow.md § The editor's controls](./game-flow.md#the-editors-controls). Exit (Esc/F10) to save - this writes the level's text `World.dat` (the editable twin of `data1.bin` - `formatWorld`/`parseWorld`).
...
- **Water** - water surfaces are **semi-transparent faces in the terrain project** (opaque = land, non-opaque = water; there is no water plane), so lakes at any altitude are just face patches at the right height. Two rules: place each water face **above a land face** (the engine pairs them by X/Z vertex overlap at load, 1.0-unit tolerance), and **never let two same-class faces coincide vertex-for-vertex in X/Z** - stacked faces, perfectly vertical walls, and sub-unit sliver triangles all trip the fatal `two land faces or two sea faces` assert (only checked when `wcache.bin` regenerates; see formats.md § Water). `REFSetWater(a)` only sets the wave-bob amplitude. See `formats.md` § Water.
